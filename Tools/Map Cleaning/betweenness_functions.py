...
def turn_penalty_value(self, previous_node, current_node, next_node):
    ...
    if angle > 45:
        # print(f"{previous_node = }\t{current_node = }\t{next_node = }\t{angle = }")
        return 30.0 #62.3 enable this as a parameter
...
def scan_for_intersections(node_gdf=None, edge_gdf=None, node_snapping_tolerance=1):
    ...
    for edge_id, edge in edge_gdf.iterrows():
        ...
        intersection_filter = edge_gdf["geometry"].intersects(edge["geometry"])
        ...
        if len(intersecting_edge_ids) > 0:
            for intersecting_edge_id in intersecting_edge_ids:
                ...
                intersection_point = edge["geometry"].intersection(intersecting_edge["geometry"])
                ...
                segments = split(
                    geo.MultiLineString([edge["geometry"], intersecting_edge["geometry"]]),
                    intersection_point
                )
                #print(segments)
                gpd.GeoDataFrame(geometry=list(segments), crs=edge_gdf.crs)
                ...
                node_gdf, edge_gdf = insert_nodes_edges(
